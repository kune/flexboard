...
#### Freehand drawing canvas — attachments with transclusion

Drawings are stored as **named attachments** on a card, separate from the Markdown text. This mirrors the Obsidian model: each drawing is an independent object with a name; Markdown fields reference it by name.

**Attaching and editing drawings:**
- The card edit view includes a **Drawings** panel listing all drawings attached to the card.
- From this panel the user can: create a new drawing (opens the Excalidraw canvas, prompts for a name), open an existing drawing for editing, rename a drawing, or delete it.
- Creating or editing a drawing opens a full-page Excalidraw canvas overlay. Saving writes the updated scene back to the attachment; cancelling discards the session.
- `editor` and `owner` roles may create, modify, and delete drawings; `viewer` role has read-only access to the rendered output.

**Transcluding drawings in Markdown:**
- Any Markdown field (card description, `markdown`-typed attributes, comments) may reference an attached drawing using wikilink transclusion syntax: `![[my-diagram.excalidraw]]`
- In **view mode** the transclusion is replaced by a static SVG rendering of the named drawing. If no drawing with that name exists on the card, a "Drawing not found" placeholder is shown.
- In **edit mode** the transclusion syntax appears as plain text in the textarea; the live preview pane renders the SVG. Clicking the SVG preview opens the drawing editor for that attachment.
- The same drawing may be transcluded in multiple fields (e.g., both description and acceptance criteria).

**Data model:**
- Drawings are a sub-resource of a card: `GET/POST/PATCH/DELETE /api/v1/boards/:boardId/cards/:cardId/drawings/:drawingId`
- Each drawing stores: `id`, `name`, `excalidraw` (scene JSON), `svg` (cached SVG string, regenerated on save), `createdAt`, `updatedAt`.
- Deleting a drawing does not modify any Markdown field that references it — the transclusion simply renders the "not found" placeholder until the reference is removed.
