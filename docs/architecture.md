...
> **Version:** 0.9  
> **Date:** 2026-04-09  
> **Status:** Updated — FR-10: responsive design and touch support; ADR-14 added
...
| ADR-14 | Mobile Kanban layout | **Horizontally scrollable column strip with CSS scroll snap** | On mobile (`< 640px`), displaying all columns side-by-side in a fixed-width flex row results in unreadable cramped cards. Three alternatives considered: (1) stack columns vertically — requires heavy scrolling and loses the spatial Kanban metaphor; (2) accordion-style collapse — hides columns out of sight, increases tap count; (3) horizontal scroll snap — each column occupies the full viewport width, swipe left/right to navigate, column indicator dots show position. Option 3 was chosen because it preserves the Kanban mental model, feels native on touch screens, and requires no new components — only CSS changes (`overflow-x: auto`, `scroll-snap-type: x mandatory`, `scroll-snap-align: start` on each column). The indicator dots (one per column, active dot highlighted) are rendered in a thin strip below the board header. No JS carousel library needed. |
